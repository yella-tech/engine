...
import type { Run, ProcessState, ProcessDefinition, EffectRecord, EngineMetrics, EngineObservabilityQuery, EngineObservabilityReport, EngineStreamEvent, EventGraph, RunStatus, EngineEvent } from '../types.js'
...
function defaultBucketMsForWindow(windowMs: number): number {
  if (windowMs <= 60 * 60_000) return 5 * 60_000
...
export function registerRoutes<T extends Hono>(app: T, engine: RoutableEngine) {
  ...
  .get('/overview', (c) => {
    ...
    const observability = observabilityWindowMs
      ? engine.getObservability({ from: Math.max(0, Date.now() - observabilityWindowMs), to: Date.now() })
      : null
